...
def main():
  for model_name in model_names:
    ...
    for i2, filter2 in enumerate(filters):
      fpr = overlap_filters(filter_lists, filter1,filter2)

      print(filter1, filter2, fpr)
...
def overlap_filters(filter_lists, f1,f2):
  if f1 == f2: return 100
  f1_list = filter_lists[f1].copy()
  f2_list = filter_lists[f2].copy()
  overlap = 0

  for entity in filter_lists[f2].copy():
    if entity in f1_list: overlap +=1

  if overlap == 0: return 0
  print("\n", overlap, len(f2_list))

  return round(100*(float(len(f2_list) - overlap)/len(f2_list)), 1)
