...
class CoreApiRequestor:
    ...
    def get_base_query(self, method, query, fulltext, max_pages=100):
        ...
        if (result['totalHits'] > self.pagesize):
            num_pages = int(result['totalHits'] / self.pagesize) + (
                        result['totalHits'] % self.pagesize != 0)  # rounds down
            if (num_pages > max_pages):
                print("Search exceeded maximum pages (%d > %d), consider breaking it down into finer grained parts" % (
                num_pages, max_pages))
                num_pages = max_pages
